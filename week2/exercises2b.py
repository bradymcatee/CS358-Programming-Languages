...
def uniquifyEnv[V](env: Env[V]) -> Env[V] :
    ...
    # Iterate throgh the list, adding bindings if they do not already appear in our new list
    unique_bindings = []
    seen = set()
    for (n,v) in env:
        if n not in seen:
            unique_bindings.append((n,v))
            seen.add(n)
    return tuple(unique_bindings)
...
def simplifyInEnv(env: Env[Expr], e: Expr) -> Expr:
    ...
    match e:
        ...
        case Lit(b):
             return Lit(b)
        case Name(n):
             return simplifyInEnv(env, lookupEnv(n,env))
        case Let(n,d,b):
            simplified_def = simplifyInEnv(env, d)
            newEnv = extendEnv(n, simplified_def, env)
