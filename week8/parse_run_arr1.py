from interp_arr1 import Add, Sub, Mul, Div, Neg, Let, Name, Lit, Ifnz, Letfun, App, Arr, Get, Set, Seq, Expr, run
...
parser = Lark(Path('expr_arr1.lark').read_text(),start='expr',ambiguity='explicit')
# for static ambiguity checking (needs package lark[interegular])
# parser = Lark(Path('expr_arr.lark').read_text(),start='expr',parser='lalr',strict=True) 
...
class ToExpr(Transformer[Token,Expr]):
    ...
    def args(self,myargs:list[Expr]) -> list[Expr]:
        return myargs
    def arr(self,args:tuple[list[Expr]]) -> Expr:          
        return Arr(args[0])
    def get(self,args:tuple[Expr,Expr]) -> Expr:
        return Get(args[0],args[1])
    def set(self,args:tuple[Expr,Expr,Expr]) -> Expr:
